...
sub default : StartRunmode {
    my $self = shift;
    my $q = $self->query;
    my $t = $self->load_tmpl('roles/default.tpl');
    my $total_roles =  $self->dbh->selectrow_array('select count(*) from roles');
    $t->param(total_roles => $total_roles);
    return $t->output;
}
...
sub edit_display : Runmode {
    ...
    my $t = $self->load_tmpl('roles/edit.tpl');

    $self->_load_schema();
    my $rs =
      $self->{schema}->resultset('Roles')
      ->search( $search_constraint, { columns => [qw/id name/], }, );
  
    my $result = $rs->first;
    if (! defined $result){
        $self->param('report_error', 'The role you are trying to edit does not exists!');
        return $self->forward('report_error');
    }
...
sub edit_process : Runmode {
    ...
    $self->session->param(flash_msg => 'Role updated!');
    $self->redirect($self->config_param('app_base_url').'/roles/show/'.$id_p);
}
...
sub list : Runmode {
    ...
    my $t = $self->load_tmpl('roles/list.tpl');
    $self->_load_schema();
    my $rs =
      $self->{schema}->resultset('Roles')
      ->search( $search_constraint, { columns => [qw/id name/], }, );
...
sub show : Runmode {
    ...
    my $t = $self->load_tmpl('roles/show.tpl');
